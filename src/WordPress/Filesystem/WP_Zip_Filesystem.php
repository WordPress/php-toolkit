...
class WP_Zip_Filesystem extends WP_Abstract_Filesystem {
	...
	public function next_file_chunk() {
		...
		if ( NewZipStreamReader::STATE_FILE_ENTRY !== $this->zip->get_state() ) {
			return false;
		}
	...
	private function load_central_directory() {
		...
		while($this->zip->next()) {
			if(NewZipStreamReader::STATE_CENTRAL_DIRECTORY_ENTRY !== $this->zip->get_state()) {
				continue;
			}
	...
	private function collect_central_directory_end_header() {
		...
		if($this->zip->get_state() !== NewZipStreamReader::STATE_END_CENTRAL_DIRECTORY_ENTRY) {
			return false;
		}
