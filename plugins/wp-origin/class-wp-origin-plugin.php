...
class WP_Origin_Plugin {
		...
		try {
			...
			if ( self::is_push_request( $git_path ) ) {
				try {
					...
					$repository->set_branch_tip( 'refs/heads/' . self::DEFAULT_BRANCH, $push_header['old_oid'] );
	...
	private static function open_repository() {
		$repository = new GitRepository(
			LocalFilesystem::create( self::get_repository_path() ),
			array(
	...
	private static function get_repository_path() {
		$upload_dir = wp_upload_dir();

		return $upload_dir['basedir'] . '/wp-origin/repository';
	}
	...
	private static function path_to_post_type( $path ) {
		...
		if ( empty( $segments[0] ) || ! in_array( $segments[0], array( 'post', 'page' ), true ) ) {
