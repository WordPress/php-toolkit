...
export function parseFragment(html: string, handler: Partial<TokenHandler>) {
    const noopHandler: TokenHandler = {
        onStartTag: (token: Token.TagToken) => {},
        onEndTag: (token: Token.TagToken) => {},
        onComment: (token: Token.CommentToken) => {},
        onDoctype: (token: Token.DoctypeToken) => {},
        onEof: (token: Token.EOFToken) => {},
        onCharacter: (token: Token.CharacterToken) => {},
        onWhitespaceCharacter: (token: Token.CharacterToken) => {},
        onNullCharacter: (token: Token.CharacterToken) => { },
    };
    const parser = new ParserFeedbackSimulator(
        {},
        {
            ...noopHandler,
            ...handler,
