...
class WXR_Parser_Regex {
	...
	public function parse( $file ) {
		...
		$multiline_tags = array(
			...
			'wp:wp_author' => array( 'authors', array( $this, 'process_author' ) ),
		);
	...
	public function get_tag( $text, $tag ) {
		if ( $tag === 'wp:author_login' ) {
			preg_match( '|<wp:author_login>(.*?)</wp:author_login>|is', $text, $return );
			if ( ! isset( $return[1] ) ) {
				preg_match( '|<wp:wp_author_login>(.*?)</wp:wp_author_login>|is', $text, $return );
			}
		} else {
	...
	public function process_author( $a ) {
		return array(
			...
			'author_login'        => $this->get_tag( $a, 'wp:author_login' ) ?: $this->get_tag( $a, 'wp:wp_author_login' ),
	...
	public function process_post( $post ) {
		...
		if ( ! isset( $this->authors[ $post_author ] ) ) {
			$this->authors[ $post_author ] = array(
				'author_login' => $post_author,
			);
		}
