...
class NativeHTMLConformanceTest extends TestCase {
	...
	public function test_public_html_classes_use_native_processors_when_available() {
		if ( ! class_exists( 'WP_HTML_Native_Tag_Processor', false ) || ! class_exists( 'WP_HTML_Native_Processor', false ) ) {
	...
	public function test_public_html_classes_can_disable_native_defaults_by_constant() {
		...
		$this->assertSame(
			...
			$this->run_html_native_defaults_constant_probe( "define( 'WP_NATIVE_APIS_ENABLE_HTML_DEFAULTS', false );" )
		);
	}

	/**
	 * Verifies HTML native defaults can be explicitly enabled by constant.
	 */
	public function test_public_html_classes_can_enable_native_defaults_by_constant() {
		if ( ! class_exists( 'WP_HTML_Native_Tag_Processor', false ) || ! class_exists( 'WP_HTML_Native_Processor', false ) ) {
			$this->markTestSkipped( 'Native HTML classes are not registered; load the native API extension to run this case.' );
		}
		...
		$this->assertSame(
			array(
				'tag:WP_HTML_Native_Tag_Processor',
				'tag-id:1',
				'tree:WP_HTML_Native_Processor',
				'tree-token:P',
				'full:php',
				'full-token:HTML',
			),
			$this->run_html_native_defaults_constant_probe( "define( 'WP_NATIVE_APIS_ENABLE_HTML_DEFAULTS', true );" )
		);
	}

	/**
	 * Verifies the global native-default kill switch takes precedence.
	 */
	public function test_global_native_defaults_disable_overrides_html_enable_constant() {
		if ( ! class_exists( 'WP_HTML_Native_Tag_Processor', false ) || ! class_exists( 'WP_HTML_Native_Processor', false ) ) {
			$this->markTestSkipped( 'Native HTML classes are not registered; load the native API extension to run this case.' );
		}
		...
		$this->assertSame(
			...
			$this->run_html_native_defaults_constant_probe(
				"define( 'WP_NATIVE_APIS_DISABLE_DEFAULTS', true );\n" .
				"define( 'WP_NATIVE_APIS_ENABLE_HTML_DEFAULTS', true );"
			)
		);
	}

	/**
	 * Verifies HTML native defaults can be disabled by environment.
	 */
	public function test_public_html_classes_can_disable_native_defaults_by_environment() {
		if ( ! class_exists( 'WP_HTML_Native_Tag_Processor', false ) || ! class_exists( 'WP_HTML_Native_Processor', false ) ) {
			$this->markTestSkipped( 'Native HTML classes are not registered; load the native API extension to run this case.' );
		}

		$previous_enable_html_defaults = getenv( 'WP_NATIVE_APIS_ENABLE_HTML_DEFAULTS' );
		$disabled_values               = array( '0', 'false', 'FALSE', 'no', 'No', 'off', 'OFF' );

		try {
			foreach ( $disabled_values as $disabled_value ) {
				putenv( 'WP_NATIVE_APIS_ENABLE_HTML_DEFAULTS=' . $disabled_value );

				$tag_processor = new WP_HTML_Tag_Processor( '<p data-id="1">Text</p>' );
				$this->assertNull( $this->get_native_delegate( $tag_processor ), "Expected disabled value {$disabled_value} to force PHP-backed tag processing." );
				$this->assertTrue( $tag_processor->next_tag( 'p' ) );
				$this->assertSame( '1', $tag_processor->get_attribute( 'data-id' ) );

				$html_processor = WP_HTML_Processor::create_fragment( '<p>Text</p>' );
				$this->assertNull( $this->get_native_delegate( $html_processor ), "Expected disabled value {$disabled_value} to force PHP-backed tree processing." );
				$this->assertTrue( $html_processor->next_token() );
				$this->assertSame( 'P', $html_processor->get_token_name() );

				$full_processor = WP_HTML_Processor::create_full_parser( '<html><body><main>Text</main></body></html>' );
				$this->assertNull( $this->get_native_delegate( $full_processor ), "Expected disabled value {$disabled_value} to force PHP-backed full-document processing." );
				$this->assertTrue( $full_processor->next_token() );
				$this->assertSame( 'HTML', $full_processor->get_token_name() );
			}
		} finally {
			if ( false === $previous_enable_html_defaults ) {
				putenv( 'WP_NATIVE_APIS_ENABLE_HTML_DEFAULTS' );
			} else {
				putenv( 'WP_NATIVE_APIS_ENABLE_HTML_DEFAULTS=' . $previous_enable_html_defaults );
			}
		}
	}

	/**
	 * Verifies HTML native defaults can be explicitly enabled by environment.
	 */
	public function test_public_html_classes_can_enable_native_defaults_by_environment() {
		if ( ! class_exists( 'WP_HTML_Native_Tag_Processor', false ) || ! class_exists( 'WP_HTML_Native_Processor', false ) ) {
			$this->markTestSkipped( 'Native HTML classes are not registered; load the native API extension to run this case.' );
		}

		if ( defined( 'WP_NATIVE_APIS_DISABLE_DEFAULTS' ) && WP_NATIVE_APIS_DISABLE_DEFAULTS ) {
			$this->markTestSkipped( 'WP_NATIVE_APIS_DISABLE_DEFAULTS is defined; HTML native defaults cannot be enabled in this process.' );
		}
		...
		try {
			foreach ( $enabled_values as $enabled_value ) {
				putenv( 'WP_NATIVE_APIS_ENABLE_HTML_DEFAULTS=' . $enabled_value );
