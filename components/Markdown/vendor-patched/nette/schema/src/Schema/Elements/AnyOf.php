...
final class AnyOf implements Schema
{
 ...
 public function __construct(...$set)
	{
		if (!$set) {
			throw new VendorPrefix\Nette\InvalidStateException('The enumeration must not be empty.');
		}
	...
	{
		$this->set[] = new Type(VendorPrefix\Nette\Schema\DynamicParameter::class);
		return $this;
	}
 ...
 private function findAlternative($value, Context $context)
	{
		...
		foreach ($this->set as $item) {
			if ($item instanceof Schema) {
				...
				$expecteds[] = VendorPrefix\Nette\Schema\Helpers::formatValue($item);
			}
		}
		...
		if ($innerErrors) {
			...
			$context->addError('The %label% %path% expects to be %expected%, %value% given.', VendorPrefix\Nette\Schema\Message::TypeMismatch, [
					'value' => $value,
